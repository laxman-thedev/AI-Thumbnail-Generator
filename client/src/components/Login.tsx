...
/**
 * Login - Authentication page component supporting both login and signup.
 * Toggles between "login" and "register" modes. On successful auth,
 * redirects the user to the home page via useEffect watching the user state.
 */
const Login = () => {

    // Toggles between "login" and "register" form modes
    const [state, setState] = useState("login")

    const {user,login,signUp} = useAuth()
    const navigate = useNavigate();

    // Controlled form fields for name, email, and password
    ...
    /** Submit the form: calls login or signUp based on current state */
    ...
    // Redirect to home when the user becomes authenticated
   useEffect(() => {
        if(user){
            navigate('/')
        }
